...
static char* find_chars_or_comment(const char* s, const char* chars)
{
    ...
    while (*s && (!chars || !strchr(chars, *s)) &&
           !(was_space && (strchr(INI_INLINE_COMMENT_PREFIXES, *s) || CUSTOMIZE_COMMENT(s)))) {
...
#define HANDLER(u, s, n, v) handler(u, s, n, v)
    ...
    while (reader(line, max_line, stream) != NULL) {
        ...
        if (strchr(INI_START_COMMENT_PREFIXES, *start) || CUSTOMIZE_COMMENT(start)) {
            ...
            if (*end == ']') {
                while (*end == ']' && *(end + 1) == '[') {
                    *end = SUBSECTION_SEPARATOR;
                    *(end + 1) = SUBSECTION_SEPARATOR;
                    end = find_chars_or_comment(end + 1, "]");
                }
                *end = '\0';
